...
contract MemeFactory {
    ...
    constructor(address _projectOwner) {
        ...
        implementation = new MemeToken(); // 部署模板合约
    }
    ...
    function deployMeme(
        ...
        // 校验参数
        require(totalSupply > 0, "MemeFactory: total supply must be positive");
        require(perMint > 0, "MemeFactory: per mint must be positive");
        require(totalSupply % perMint == 0, "MemeFactory: total supply must be divisible by per mint");
        require(price > 0, "MemeFactory: price must be positive");

        // 创建最小代理
        address proxy = _createProxy();
        
        // 初始化代理（调用模板合约的initialize函数）
        MemeToken(proxy).initialize(
            name,
            symbol,
            totalSupply,
            perMint,
    ...
    function mintMeme(address tokenAddr) external payable {
        // 校验代币是否存在
        require(tokenToIssuer[tokenAddr] != address(0), "MemeFactory: invalid token");
        
        MemeToken token = MemeToken(tokenAddr);
        address issuer = tokenToIssuer[tokenAddr];
        
        // 校验支付金额
        uint256 requiredPayment = token.perMint() * token.price();
    ...
    function _createProxy() internal returns (address) {
        ...
        require(proxy != address(0), "MemeFactory: proxy creation failed");
        return proxy;
    }
}